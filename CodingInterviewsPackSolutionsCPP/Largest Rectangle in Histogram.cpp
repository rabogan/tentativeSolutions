...
b) We want the 1st position BEFORE i SMALLER than height[i]
c) We want the 1st position AFTER i SMALLER than height[i]
c-b gives the LENGTH of our rectangle. The height is height[i]
e.g. for this array {4,7,5,9,5,8,2,1}
At index i = 2 (value 5)
a) is 5
b) is i = 0  (Position 0.  4 is the first value BEFORE 
i where the height is less than height[i])
c) is i = 6  (Position 6.  2 is the first value AFTER 
i where the height is less than height[i])

To get c (the NEXT smaller element for each position), 
we use a variation on Next Greater Element's stack approach, 
simply next_smaller_element!
To get b (the previous smaller element for each position), 
we need to modify things a little
i) Make a copy of the heights vector
ii) Reverse it
iii) Run the next_smaller_element function on it!
iv) Reverse that result
v)  Using result[i] =  n-result[i]-1, ensure the indices 
are indeed showing the PREVIOUS smaller element
...
3) We iterate through our heights, and calculate an INITIAL 
AREA for a rectangle at height[i]
4) We maximize among these areas to get our result, returning 
(in my exemplar case) maxArea
*/
