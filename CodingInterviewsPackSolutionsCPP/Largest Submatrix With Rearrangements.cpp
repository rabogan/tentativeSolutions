...
public:
    ...
    vector<int>PSI(const vector<int>& v) {
        vector<int> res = v;
...
OUR GOAL:
We want to get the maximum area of 1s possible, and can/must 
use rearrangement of the columns to do so.  Simply leaving the 
matrix as it is may not give the right solution

OUR SOLUTION:
1) Look at how we can represent this matrix as a HISTOGRAM:
...
c) We iterate through our heights, and calculate an INITIAL 
AREA for a rectangle at height[i]
d) We maximize among these areas to get our result, 
returning (in my exemplar case) maxArea
We will SLICE UP the matrix, going from the top row to 
the bottom row, calculating areas!

2) HOWEVER, we need to perform a special prefix sum 
calculation, so that the LOWER ROWS are well represented.
The details about this are similar to LeetCode 85 (HERE):
...
c)  Perform a special function, prefix_sum_column_variant, 
to change our integer matrix
...
3) BUT, we are allowed AND MUST sort out our rows, row by 
row, each time.  This will coagulate all our largest values 
to the RIGHT HAND SIDE...e.g.
...
Can you visually observe our answer here?
12!!! (4*3)--->plus row number 3 (0,1,2,3,4) into the 
